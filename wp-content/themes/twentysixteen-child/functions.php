...
function my_acf_options_page_settings( $settings )
{
	...
	$settings['pages'] = array('Header','Footer','Team','Privacy Policy','Language');
    return $settings;
}
...
// CUSTOM EXCERPT FOR TRIMMING BLOG STYLE SUMMARIES - | ALLOWS SPECIFIED HTML elements
...
/* add for navigation b/w country*/
add_action('wp_ajax_get_curl', 'get_curl');
add_action('wp_ajax_nopriv_get_curl', 'get_curl');
function get_curl($page_slug){
	//echo $_POST['page_slug'];

    $site_url = $_POST['page_slug'];
 $ch = curl_init();
...
// LOad more Functionality for Blog page

add_action('wp_ajax_load_More_blog', 'load_More_blog');
add_action('wp_ajax_nopriv_load_More_blog', 'load_More_blog'); // not really needed
function load_More_blog() {
    global $wpdb;
    //print_r($_POST);
    extract($_POST);

            $args = array ('post_type'=> array( 'post' ),'post_status'=> array( 'publish' ),'posts_per_page'=> '5','offset'=>$offset);
...
/*Post type for Privacy Policy section*/

 // Register Custom Post Type
function privacy_policy() {

	$labels = array(
		'name'                  => _x( 'Privacy Policy', 'Post Type General Name', 'text_domain' ),
		'singular_name'         => _x( 'Privacy', 'Post Type Singular Name', 'text_domain' ),
		'menu_name'             => __( 'Privacy Policy', 'text_domain' ),
		'name_admin_bar'        => __( 'Post Type', 'text_domain' ),
		'archives'              => __( 'Item Archives', 'text_domain' ),
		'attributes'            => __( 'Item Attributes', 'text_domain' ),
		'parent_item_colon'     => __( 'Parent Item:', 'text_domain' ),
	...
	register_post_type( 'privacy_policy', $args );

}
add_action( 'init', 'privacy_policy', 0 );
...
/*Post type for library section*/

 // Register Custom Post Type
function library() {

	$labels = array(
		'name'                  => _x( 'Library', 'Post Type General Name', 'text_domain' ),
		'singular_name'         => _x( 'Library', 'Post Type Singular Name', 'text_domain' ),
...
<a id="<?php echo str_replace($site_url,'',get_permalink());  ?>" class="confirmform" name="<?php echo get_the_title(); ?>"
...
add_action('wp_ajax_ajax_search', 'ajax_search' ); // executed when logged in
add_action('wp_ajax_nopriv_ajax_search', 'ajax_search' ); // executed when logged out

/*LOad more Press section*/

add_action('wp_ajax_load_More_press', 'load_More_press');
add_action('wp_ajax_nopriv_load_More_press', 'load_More_press'); // not really needed
function load_More_press() {
    global $wpdb;
    //print_r($_POST);
    extract($_POST);

            $args = array ('post_type'=> array( 'press' ),'post_status'=> array( 'publish' ),'posts_per_page'=> '5','offset'=>$offset);
            $query = new WP_Query( $args );
            if( $query->have_posts() ){
            	while ( $query->have_posts() ) : $query->the_post();
							$site_url=site_url(); ?>
   ...
   function Get_Custom_Data()
    {
     ...
     global $wpdb;
     // $get_data=$wpdb->get_results( 'SELECT IPAddress FROM CustomCheck where PostID=$id ');
     // echo $get_data;die;
    //if($get_data == 0){

     /*INsert data if these values not exist in database*/
      $query= $wpdb->insert('CustomCheck', array('PostID' =>$id , 'IPAddress' =>$IPAddress ,'Status'=>$status) );

      /*Check all entries if same ipaddress belongs to more then one day then it will be delete automatically */
     $Del_Query=$wpdb->query('DELETE FROM CustomCheck WHERE Time < NOW() - INTERVAL 24 HOUR');
...
add_action('wp_ajax_Get_Custom_Data', 'Get_Custom_Data' ); // executed when logged in
add_action('wp_ajax_nopriv_Get_Custom_Data', 'Get_Custom_Data' ); // executed when logged out
...
function btn_shortcode( $atts, $content = null ) {
    $a = shortcode_atts( array(
        'class' => 'btn btn-understand-stroke btn-pill-large learn-more-btn add_content_id_mk',
        'href'  =>  '#'
    ), $atts );

    return '<a class="' . esc_attr($a['class']) . '"  onclick="show_form()" >' . $content . '</a>';
}
